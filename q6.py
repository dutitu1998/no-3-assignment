...
def linear_shooting(h):
    ...
    n = int((b - a) / h)
    # Create the mesh
    x = np.linspace(a, b, n+1) # 10 interval means 11 points,x=n+1
    ...
    y2 = sol2.y[0]
    c = (beta - y1[-1]) / y2[-1] #In Python, y1[-1] refers to the last element of the array y1.
...
def bc(ya, yb):
    return np.array([ya[0] - 1, yb[0] - np.exp(-10)]) #Define a boundary condition function bc(ya, yb) that returns how far the current solution is from satisfying the boundary conditions.
...
y_bvp_fine = sol_bvp.sol(x_fine)[0]
y_exact_fine = exact_solution(x_fine)
error_bvp = np.max(np.abs(y_bvp_fine - y_exact_fine))
...
plt.subplot(2, 2, 3)
plt.plot(x_fine, y_bvp_fine, 'mo-', label='solve_bvp')
...
plt.semilogy(x_fine, np.abs(y_bvp_fine - y_exact_fine), 'm-', label='Error (solve_bvp)')
